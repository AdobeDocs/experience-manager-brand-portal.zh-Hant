...
1. 在工具列中按一下&#x200B;**[!UICONTROL 搜尋]**&#x200B;圖示，或按下&quot;**[!UICONTROL /]**&quot;鍵以啟動Omnisearch。
...
1. 在搜尋方塊中，輸入您要搜尋之資產的關鍵字。
   ...
   >* 搜尋`mountain biking`時，Omnisearch會傳回搜尋結果中所有在中繼資料欄位中同時具有`mountain`和`biking`的資產。 例如，`Title`欄位中的`mountain`和`Description`欄位中的`biking`。 這兩個字詞都必須在中繼資料欄位中可用，才能顯示在搜尋結果中。 不過，Omnisearch會在搜尋結果中傳回資產，即便智慧型標籤中繼資料欄位中只有兩個辭彙可供使用。 例如，如果資產包含`mountain`作為其中一個智慧標籤，而在任何其他中繼資料欄位中不包含`biking`，而您搜尋`mountain biking`，則omnisearch仍會在搜尋結果中傳回資產。
...
若要瞭解智慧標籤資產的搜尋行為，請參閱[瞭解搜尋結果和行為](https://experienceleague.adobe.com/docs/experience-manager-65/assets/using/search-assets.html)。

## 在「篩選器」面板中使用Facet進行搜尋 {#search-using-facets-in-filters-panel}

「篩選器」面板中的搜尋Facet可為您的搜尋體驗增加精細度，並讓搜尋功能有效率。 搜尋Facet會使用多個維度（述詞），讓您執行複雜的搜尋。 您可以輕鬆向下展開至所需的詳細資訊層級，以更集中地搜尋。

例如，如果您要尋找影像，您可以選擇您要點陣圖或向量影像。 您可以在「檔案型別」搜尋面向中指定影像的MIME型別，進一步縮小搜尋範圍。 同樣地，在搜尋檔案時，您可以指定格式，例如PDF或MS® Word格式。

Brand Portal中的![篩選器面板](assets/file-type-search.png "Brand Portal中的篩選器面板")

**[!UICONTROL 篩選器]**&#x200B;面板包含幾個標準Facet，例如 — **[!UICONTROL 路徑瀏覽器]**、**[!UICONTROL 檔案型別]**、**[!UICONTROL 檔案大小]**、**[!UICONTROL 狀態]**&#x200B;以及&#x200B;**[!UICONTROL 方向]**。 不過，您可以在基礎搜尋表單中新增或移除述詞，以[新增自訂搜尋Facet](../using/brand-portal-search-facets.md)或從&#x200B;**[!UICONTROL 篩選器]**&#x200B;面板中移除特定搜尋Facet。 檢視Brand Portal](../using/brand-portal-search-facets.md#list-of-search-predicates)上可用和可用的[搜尋述詞清單。
...
例如，使用下列標準篩選器：
   ...
   * **[!UICONTROL 路徑瀏覽器]**&#x200B;以搜尋特定目錄中的資產。 路徑瀏覽器述詞的預設搜尋路徑為`/content/dam/mac/<tenant-id>/`，可透過編輯預設搜尋表單來設定。
   ...
   * **[!UICONTROL 屬性述詞]**&#x200B;若用於搜尋表單，可讓您搜尋符合述詞對應之中繼資料屬性的資產。\
     ...
     **部分片語**
若要允許以屬性述詞中的部分片語搜尋資產，請啟用[搜尋表單]中的**[!UICONTROL 部分搜尋]**&#x200B;核取方塊。 如此一來，便可讓您搜尋所需資產，即使您未指定資產中繼資料裡所使用的確切單字/詞組。
     ...
     您可以：
      * 在「篩選器」面板的Facet中，指定搜尋片語中的單字。 例如，如果您搜尋字詞&#x200B;**climb** （且「屬性述詞」對應至[!UICONTROL `dc:title`]屬性），則系統會傳回標題片語中包含&#x200B;**climb**&#x200B;字的所有資產。
...
例如，搜尋：
     ...
     **不區分大小寫的文字**
若要允許在屬性述詞中進行不區分大小寫的搜尋，請啟用[搜尋表單]中的**[!UICONTROL 忽略大小寫]**&#x200B;核取方塊。 依預設，屬性述詞的文字搜尋會區分大小寫。
...
您可以將搜尋設定儲存為智慧型集合，以便能夠快速重複相同的搜尋，而不需要稍後重做相同的設定。 不過，您無法在集合中套用搜尋篩選器。
...
1. 點選/按一下&#x200B;**[!UICONTROL 儲存智慧型集合]**，並提供智慧型集合的名稱。
   ...
   >[!NOTE]
   >
   >非管理員使用者可將智慧型集合限製為公開，以避免非管理員使用者在組織的Brand Portal上建立大量公開的智慧型集合。 組織可以停用管理工具面板中可用之&#x200B;**[!UICONTROL 一般]**&#x200B;設定中的&#x200B;**[!UICONTROL 允許公用智慧型集合建立]**&#x200B;設定。
